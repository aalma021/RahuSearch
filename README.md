...
## 2) Update / Add New Data (Incremental)

This step is used when new product data is added after the initial pre-deploy.

- Does NOT recreate the index
- Does NOT remove existing data
- Safely inserts or updates records

### Steps

1) Update your dataset under the directory defined by `DATA_ROOT` in `.env`.
...
If both flags are `false`, only new products are inserted.

This command can be safely run multiple times.


---

## 3) Backend API (Run Application Anytime)
